...
@MainActor private var rects : (img: CGRect, title: CGRect, info: CGRect, width: Int)? = nil
...
//            info.minimumScaleFactor = 10.0
        ...
        // Image config
        if let logodata = manager.wxcList[index].logodata as? Data {
            img.image = UIImage(data: logodata)
        } else {
            img.image = nil
        }
